...
GA4 BigQuery 데이터를 기반으로 이벤트별 마케팅 성과를 자동 집계하고, Vertex AI로 실행 액션 중심의 인사이트를 생성하는 대시보드 프로젝트입니다.

이 프로젝트는 단순히 수치를 보여주는 리포트가 아니라,  
...
| 항목 | 내용 |
|---|---|
| 목적 | 이벤트별 마케팅 성과 진단 및 액션 추천 자동화 |
| 데이터 | GA4 Export 기반 BigQuery 데이터 |
| 분석 단위 | 3개월, 1개월, 주차별 이벤트 성과 |
| AI 활용 | Vertex AI Gemini로 이벤트별 요약 및 액션 생성 |
| 시각화 | Looker Studio 대시보드 |
| 자동화 | Cloud Run Jobs, Cloud Scheduler |

## 핵심 결과

- 이벤트별 유입, 구매전환, 참여전환, 매출, 객단가를 자동 집계
- 전체 이벤트 내 순위와 사분면 위치를 기반으로 성과 유형 분류
- 유저 군집 분석을 통해 이벤트 방문자의 성향 반영
- Vertex AI를 활용해 이벤트별 요약과 실행 액션 자동 생성
- Looker Studio에서 기간, 이벤트코드, 생성일 기준 필터링 가능
